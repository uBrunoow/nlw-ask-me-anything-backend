...
func (h apiHandler) handleGetRooms(w http.ResponseWriter, r *http.Request) {
	...
	// Converter parâmetros de paginação para inteiros
	if pageStr != "" {
	...
	// Criar os parâmetros para GetRooms
	params := pgstore.GetRoomsParams{
		Limit:  int32(pageSize),
		Offset: int32(offset),
	}

	slog.Info("getting rooms", "params", params)

	// Obter as salas
	rooms, err := h.q.GetRooms(r.Context(), params)
	if err != nil {
		http.Error(w, "something went wrong", http.StatusInternalServerError)
